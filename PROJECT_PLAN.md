...
| 面向 | 文档 | 内容 | 主要读者 | 变更频率 |
|---|---|---|---|---|
| 总则 | [architecture-架构与原则.md](platform/docs/architecture-架构与原则.md) | 项目是什么、三条核心原则、五层架构、两个前端+共享后端、三类工具、入口门户与账号体系 | 决策者 / 新人 | 几乎不变 |
| 总则 | [execution-plan-执行计划与技术栈.md](platform/docs/execution-plan-执行计划与技术栈.md) | 分阶段计划、技术栈、`platform/` 目录结构、给 AI 的部署命令 | 建平台的团队 | 每完成一阶段 |
| 总则 | [code-standards-代码规范.md](platform/docs/code-standards-代码规范.md) | 工程纪律：单向依赖、schema 代码生成、删不注释、CI 机器闸门 | 写代码的人/AI | 偶尔 |
| 共享层 | [schema-数据契约.md](platform/docs/schema-数据契约.md) | 统一事件 Schema（三圈字段），**带契约版本号** | 开发 + 接入方 | 跟 schema_version 演进 |
| 共享层 | [registry-工具注册表.md](platform/docs/registry-工具注册表.md) | `tool_id` 来源 + 门户展示字段，两端共读 | 开发 / 接入方 / 门户 | 偶尔 |
| 共享层 | [metadata-conventions-metadata约定与字段治理.md](platform/docs/metadata-conventions-metadata约定与字段治理.md) | metadata 怎么记（统一形状）+ 字段三层模型 + 晋升流程 | 开发 + 接入方 | 跟约定演进 |
| 数据端 | [integration-guide-接入指南.md](platform/docs/integration-guide-接入指南.md) | 接入方总入口：五步流程 + 最小示例 + 边界，可直接发给接入方 | 工具方（接入方） | 偶尔 |
| 数据端 | [ai-intake-guide-AI接入向导.md](platform/docs/ai-intake-guide-AI接入向导.md) | 给接入方 AI 看的接入剧本：自助判定字段 + 反问工具方规则 + 产出接入配置/方案 | 工具方的 AI | 偶尔 |
...
| 钉钉治理 | [dingtalk-钉钉组织与部门治理/](platform/docs/dingtalk-钉钉组织与部门治理/README-总览与索引.md) | 钉钉组织同步与免登数据门禁已落地待真机验证；部门化工具可见性仍是设计稿，决策 #38/#47 | 建平台的团队 | 阶段推进时更新 |
...
> 代码区结构（`platform/`：两个前端 + 共享后端 + 集成层）见 [执行计划](platform/docs/execution-plan-执行计划与技术栈.md) 第二节。
...
14. **靠机器闸门治代码混乱**：针对「越写越乱/残留/AI 不敢删」，不只靠规范文档，而是 CI + pre-commit 强制格式化/lint/类型/**死代码扫描**/分层依赖/测试，任一不过不许合并。详见 [代码规范](platform/docs/code-standards-代码规范.md)。
...
30. **`规划/` 全量搬入 `platform/docs/`，规划目录退役（决策 #26 的 (a) 升级为全量）**：原决策 #26 只部分冻结（数据契约 / 接入契约转 platform，其余 5 份留 `规划/`），形成「两份看 platform、五份看规划」的双轨状态，对长期维护与交接是认知负担。本轮按用户「要标准、可交接」的要求，把剩余 5 份（架构与原则 → architecture-架构与原则.md、执行计划 → execution-plan-执行计划与技术栈.md、代码规范 → code-standards-代码规范.md、工具注册表 → registry-工具注册表.md、工具门户 → portal-工具门户.md）也搬入 `platform/docs/`（统一英文文件名），并删除已退役的 `规划/`（含两份冻结归档，其真源早已在 platform）。此后只有一个 SSOT 位置 `platform/docs/`，不再区分规划 / platform。所有跨文档链接同步重定向。
...
36. **`platform/docs/` 规范文档统一改名为「英文-中文.md」**：原文件名只有英文（`architecture.md` 等），在文件资源管理器里需逐个点开才知是什么。本轮按用户要求把 11 份规范文档改为「英文前缀-中文.md」（如 `architecture-架构与原则.md`、`schema-数据契约.md`），中文取自各文档一级标题/文档地图标签，**先英文后中文**——英文前缀保证开发者/脚本里仍以稳定可读 token 引用、排序友好，中文后缀让文件名一眼可读。**取舍**：中文文件名相对「纯英文」略偏离常规工程惯例，但本平台文档面向中文团队与接入方、可读性优先，且英文前缀已保留可移植性，权衡后采纳。实施＝`git mv` 改名（保留历史）+ 全仓库字面替换所有引用（含 CI/pre-commit/pyproject/多个 .py 注释/event.schema.json 注释，排除 `.pydeps` 与生成物）+ 重跑 `export_integration_kit.py` 重生成 `handoff-kit`。`README.md` 维持英文（GitHub/工具约定名，不动）。本决策只改文件名与引用，**不改任何文档内容、不升事件契约版本**。

37. **采集手段判定改为「按能力/意愿」而非「按工具来源」**：原架构第四节把「开源自部署（例 OpenClaw）」整类绑定到「**改不动代码**、只能改 base URL 走转发」，但 OpenClaw 实为完全开源、代码可改，该表述事实错误。订正：采集手段取决于**能否/愿否改这个工具的代码**——能且愿意改→工具自报（主路、数据最全）；改得动但不愿 fork 维护源码（开源自部署常见）或根本改不了（纯黑盒 SaaS）→转发/独立 key 兜底。OpenClaw 作为「开源但不想 fork、故走转发」的例子保留，仅把动机从「改不动」更正为「不想改」。进一步,用户又指出「闭源不能转发」也是同根错误——闭源工具只要暴露可配置 base URL 照样能转发。故**架构 §四整节由「按工具来源分类」重构为「按控制权三道能力闸门」**（改代码→自报 / 改 base URL→转发 / 配 key→独立 key），并写明「开/闭源不是判定依据」。本决策只订正文档表述与该表结构，不改采集架构本身（三种手段不变）、不升契约版本。连带改 `architecture-架构与原则.md` / `platform/README.md` / `collection/relay/README.md` / `contract-接入契约.md`。

38. **工具管理引入「组织维度」：钉钉组织同步 + 部门化工具治理（设计稿，待实现）**：数据接入层已闭环，但工具管理是空白——`team_id` 只是自由字符串、账号扁平无部门、门户对所有人一视同仁。本轮定方向：以公司在用的**钉钉为唯一组织源**补上「组织」这一层。经四轮澄清拍定取舍：① 核心痛点＝**权限治理**（谁能用什么）；② 强度＝**软治理**（只控门户「可见/上架」，不碰工具本身，**不破坏决策 #7 不做写入侧鉴权**）；③ 组织数据＝**钉钉同步部门树+人员**（非平台手维，避免漂移）；④ 登录＝**钉钉免登**（H5 微应用 `requestAuthCode`），密码体系（#17/#31）退居兜底；⑤ 可见性模型＝**工具挂部门白名单、默认仅本部门可见**（向下级联：开给父部门含子部门），公共工具用 `visible_to_all` 全开；⑥ 部署本轮定**只在公司网络内用 PC 钉钉**（内网可达即可，服务器需出网访问钉钉云）。**可行性已联网核实**（通讯录 API 拉部门树需从根 id=1 递归逐层、免登 `authCode` 5min 一次性换 `userid`，均为钉钉标准能力）。数据改动＝新建 `department` 表、账号加 `dingtalk_userid` + `user_department`、注册表加治理字段 `owner_dept_id`/`visible_to_all` + `tool_visible_department`，**全是注册表+账号+新表，不升事件 `schema_version`**。红利：用户有了部门后分析层可做「按部门」的四类分析。分两步建（A 钉钉打通 → B 部门化治理）。设计稿落 `platform/docs/dingtalk-钉钉组织与部门治理/`；实现时再回改 architecture §五/registry/portal/execution-plan/metadata 约定等正文（清单见该文件夹 README §六）。**原待定项 T1–T6 已人工逐项拍定（记为 P1–P6，详见 README §五）**：P1 同步频率＝每小时定时 + 管理员手动触发；**P2 密码体系完全退役**，钉钉免登为唯一登录、首个 admin 引导当时定为 seed 脚本绑定钉钉 `userid`，后续已由 #47 更正为 `BOOTSTRAP_ADMIN_DINGTALK_USERID` 配置认定超管（**此项取代决策 #17/#31 的密码登录部分**）；P3 同步未覆盖的新员工＝免登时即时拉一次当场建账号；P4 工具可见白名单＝单独关联表 `tool_visible_department`（非 JSONB）；P5 门户可达＝只在公司网络内用 PC 钉钉；P6 删除部门/离职＝软删保留（`active=false`）。
...
48. **数据分析页 AI 能力从「自由问答」改为「按工具 + 周期生成 HTML 报告并下载」**：原 `POST /ai/query` 只把全局 `analytics/summary` 聚合塞给模型，前台按钮虽叫「生成报告」，实际只是回答一句话，无法满足周期性报告的结构、取数和下载需求。本轮拍板新正路：数据分析页保留原提示词输入框与按钮排版，仅新增「选择功能」+「开始/结束日期」控件，点击后调用 `POST /ai/report`；后端按 `tool_id` 与日期范围查询本期数据，并自动生成同长度上一周期作为对比基期，聚合总请求、成功数、平均耗时、token、按日、按状态、按用户、按章节/功能维度，把这些已查询结果连同工具名/简介追加到用户输入的报告提示词后；模型返回完整 HTML 后前端自动触发下载。`/ai/query` 暂作为后端旧端点留存但前台不再使用，后续确认无外部调用后删除。APIMart 配置仍走 `.env` 的 `APIMART_API_KEY` / `APIMART_BASE_URL` / `APIMART_MODEL`，占位密钥视为未配置。报告模板骨架文件尚未接入：当前提示词明确标注“模板骨架未配置”，后续需把 `报告模板-通用版含目录.html` 纳入仓库或配置路径后再要求模板原样保留。本决策只新增读取侧聚合与 AI 生成接口，不改事件 schema，不升 `schema_version`。
...
- 真实 TCP 服务（uvicorn）那一跳本机跑不了（无 uvicorn + PyPI 被墙），到能联网 / 有 uvicorn 的机器按 `smoke-数据库冒烟.md` 原样补跑一次。
