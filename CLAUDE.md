...
- **当前最新发布**：`v0.3.0`（钉钉免登数据门禁 + AI 周期报告资产管理 + 一键启动脚本；前作 `v0.2.0` 含钉钉组织同步 Phase A 端到端打通 + Smart Tool Hub 高保真 mockup）。
...
├ docs/dingtalk-钉钉组织与部门治理/      钉钉组织同步与免登数据门禁已落地待真机验证；部门化工具可见性仍为设计稿。决策 #38/#47；4 份分册。
├ docs/specs/                          【在研设计稿目录】brainstorming 产出，按 `YYYY-MM-DD-主题-design.md` 命名；单个功能实现前的设计稿，非 SSOT 正式规范，落地后结论回写对应正文、设计稿留存备查。
└ backend / collection / shared / integrations / apps  代码：接入 API、生成模型、存储层、参考 SDK、demo、Auth API、试点模板、合并版前台（apps/web，门户+数据页，试点期正式入口）；relay/分析层仍占位。局域网部署与钉钉入口的事实只记在 execution-plan §四。
...
2. **不要擅自打兼容性补丁。** 旧逻辑、旧字段、旧脚本、旧入口如果已经不符合当前需求，默认应改成新的唯一正路，并清理会继续误导维护者的旧路径。只有在确实需要保留历史兼容（例如外部用户仍在使用、数据迁移需要过渡、线上入口不能同时切换）时，先向用户说明影响、范围、退出条件并请求确认；未经确认，不用“临时兼容 / 兜底分支 / 双路径并存”的方式绕过去。
3. **决策入账，不是入正文。** 「为什么这么定」的取舍记到 `PROJECT_PLAN` 决策记录（追加，按序）；正文只写「现在是怎样」，不堆历史争论。决策记录追加久了会变长，可定期把已被推翻的条目标「已废弃」或合并，别无限堆叠成另一种补丁。
4. **契约带版本，向后可追。** `platform/docs/schema-数据契约.md` 每次实质改动升版本号；记录里的 `schema_version` 就是据此盖章。
5. **稳定与易变分离。** 别把易变内容（字段、价格、阶段）塞进稳定文档（架构/原则），反之亦然。这是当初拆分的初衷，维护时别又混回去。
6. **占位明确。** 未定的事项统一写「**待定，需人工确认**」并说清待定的是什么，不要含糊带过、也不要擅自替用户拍板（尤其敏感内容策略、ROI 价值信号）。
7. **最小改动。** 一次只解决一件事；不顺手做无关重排，方便 review 和回溯。
8. **守则自身也要维护。** 新增一类文档、SSOT 转移、拆分结构变化时，同步更新这份 `CLAUDE.md`（§1 结构、§2/§3 表）。守则过时比没有守则更危险。
9. **日志与变更摘要分工。** `开发日志.md` 是过程账本：每次改动都写，方便追溯“今天动了哪些文件”。`CHANGELOG.md` 是能力摘要：只写阶段 / 发布级变化，方便快速理解“现在项目能做什么”。两者不要互相复制整段内容。
...
- [ ] **兼容性补丁**：没有擅自保留旧逻辑/旧入口/双路径兜底；若确实需要兼容，已先向用户说明范围、影响、退出条件并获得确认。
