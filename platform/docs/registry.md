...
> 字段定义配套见 [数据契约](schema.md)；接入义务见 [接入契约](contract.md)。
...
- **使用端**：使用者门户的工具目录底表——分类、卡片展示、局域网链接、排序都读它（见 [使用端-工具门户](portal.md)）。

> **与用户账号表分工**：用户账号表管「人」（`user_id`，见 [架构-入口门户与账号体系](architecture.md)）；本表管「工具」（`tool_id`）。
...
| `description` | 工具简介（卡片说明，也是 [AI 工具推荐](portal.md) 的语义匹配依据） |
...
| 鉴权 | 需 **admin token**（非 admin 403、无 token 401）。首个 admin 由 `tools/seed_admin.py` 离线引导（见 [架构-账号体系](architecture.md)） |
